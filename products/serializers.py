...
class ProductShopSerializer(serializers.ModelSerializer):
    """
    Product serializer with shop filter
    """
    ...
    def get_shop_products(self, shop_id):
        queryset = Product.objects.filter(shop_id=shop_id).prefetch_related("variants")
        ...
        return self.__class__(queryset, many=True, context=self.context).data
